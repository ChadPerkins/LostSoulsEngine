...
namespace LostSouls {
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			// Create a pointer to the end of the file to get its size
			in.seekg(0, std::ios::end);
			// Store the location of the pointer into result
			result.resize(in.tellg());
			// Move the pointer back to the start of the file
			in.seekg(0, std::ios::beg);
			// Read in the ammount of bytes the file takes up
		...
		{
			LS_CORE_ASSERT("Could not open file '{0}'", filepth);
		}
		return result;
	}
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto& kv : shaderSources)
		{
			...
			// Create an empty shader handle for the shader type
		...
		// Shaders are successfully compiled. Now time to link them together into a program.
